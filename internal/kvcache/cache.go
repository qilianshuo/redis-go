...
// Cache is a sequential key-value cache structure
type Cache struct {
...
func (c *Cache) GetEntity(key string) (entity *DataEntity, ok bool) {
...
func (c *Cache) PutEntity(key string, entity *DataEntity) (ok bool) {
	c.data[key] = entity
	return true
}

// PutIfAbsent inserts a key-value pair if the key does not already exist and returns true if the insertion was successful.
func (c *Cache) PutIfAbsent(key string, entity *DataEntity) (ok bool) {
...
func (c *Cache) PutIfExists(key string, entity *DataEntity) (ok bool) {
	if _, exists := c.data[key]; exists {
		c.data[key] = entity
		return true
	}
